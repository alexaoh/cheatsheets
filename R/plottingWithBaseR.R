###############################################################
# This is the R script for "Plotting in R" (Part 5, base R)
# Introduction into R programming, plotting with base R
###############################################################
...
plot(df$weight ~ df$height,
     ...
     cex.main=1.5, cex.lab=1.3, cex.sub=.5, cex.axis=1.5,
     las=1,
     pch=20,
     bty='l')
...
plot( df$weight)
# plot( df$weight, type='l')  # Line plot
...
lines( density(df$weight), col=4, lwd=2)
...
barplot(agr$x, names.arg = agr$Group.1,
        main = "Mean weight by district")
